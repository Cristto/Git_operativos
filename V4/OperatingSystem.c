...
int OperatingSystem_ObtainMainMemory(int processSize, int PID)
{
	...
	int dir_aux;
	int proceso_apto; // 1 proceso apto y 0 proceso apto por otro camino
	...
	for(i=0;i<PARTITIONTABLEMAXSIZE;i++){
		...
		if(processSize<=partitionsTable[i].size){			
			diff_aux=abs(partitionsTable[i].size-processSize); // valor absoluto de la diferencia
			tam=1; // tenemos un tamayo apto para la particion, el que sea

			if(diff_aux<=diff_final){ // para la primera vez se va a cumplir siempre				
				
				if(partitionsTable[i].PID==NOPROCESS){ // si el hueco esta libre
					ocupado=1; // tenemos un hueco libre

					if(diff_final==diff_aux){	
						dir_aux=partitionsTable[i].initAddress; // guardo en dir_aux la direccion de esta particion	
						//cuando compara con dir_final por primera vez puede darme problemas
						if(dir_aux<dir_final){ // la primera vez siempre se cumple
							particion_final=i;
							proceso_apto=1;
						}		
					}
					if(proceso_apto==0){ // solo toma valor en caso de que se cumpla la condicion de los tamayos
						particion_final=i;
					}
					diff_final=diff_aux; // diff siempre tendra el valor final 
				}												
			}							
		}		
	}	
