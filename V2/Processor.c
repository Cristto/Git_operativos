...
void Processor_InstructionCycleLoop() {
	...
	while (!Processor_PSW_BitState(POWEROFF_BIT)) {
		...
		// V2 Ej 3.c 
		//ex-n !Processor_PSW_BitState(INTERRUPT_MASKED_BIT)) //end ex-n
...
int Processor_FetchInstruction() {
	...
	if (registerCTRL_CPU && CTRL_SUCCESS) {
		...
		//V2 Ej 1
		//ex-n		
		ComputerSystem_DebugMessage(Processor_PSW_BitState(EXECUTION_MODE_BIT)?117:116,HARDWARE,Clock_GetTime());
		//end ex-n
...
void Processor_DecodeAndExecuteInstruction() {
	...
	switch (operationCode) {
		...
		case OS_INST: // Make a operating system routine in entry point indicated by operand1
			...
			// V1 Ej 16.a
			// comprueba si el procesador esta en modo protegido
			//ex-n
			if(Processor_PSW_BitState(EXECUTION_MODE_BIT)){
	...
	int i;
		...
		for (i=0;i<INTERRUPTTYPES;i++)
			...
			if (Processor_GetInterruptLineStatus(i)) {
				...
				// V1 Ej 13 Copy Accumulator register in the system stack
				//ex-n
