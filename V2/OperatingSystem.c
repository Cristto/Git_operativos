...
void OperatingSystem_HandleClockInterrupt(); //V2 Ej 2.c
...
void OperatingSystem_MoveToTheREADYState(int PID) {
	...
	// V2 Ej 8  -> comento la funcion del ejercicio 9.b de la V1
}
...
int OperatingSystem_ExtractFromReadyToRun() {
	...
	int queueID=processTable[selectedProcess].queueID; // V1 Ej 11.c	
...
void OperatingSystem_TerminateProcess() {
	...
	// V1 Ej 14 mirarlo
	if (numberOfNotTerminatedUserProcesses==0) {
		if (executingProcessID==sipID) { // si el que se esta ejecutando es el daemmon
...
void OperatingSystem_HandleSystemCall() {
	...
	switch (systemCallID) {
		...
		case SYSCALL_YIELD: // V1 Ej 12	
			//sacando el qeueID obtengo de que cola se esta ejecutando
			// y asi puedo comparar bien dos dos procesos de la misma cola
			...
			//de la misma cola que el proceso que se esta ejecutando 
			//solo retorno el elemento mas prioritario pero sin sacarlo
			...
			// el proceso sale del procesador y vuelve a la cola
			OperatingSystem_PreemptRunningProcess();
			//saco el proceso de cola que previamente se agrego de nuevo para que 
			//no se pueda volver a ejecutar
		...
		Heap_add(executingProcessID, sleepingProcessesQueue,QUEUE_WAKEUP ,
...
void OperatingSystem_InterruptLogic(int entryPoint){
	switch (entryPoint){
		...
		//V2 Ej 2.c
		//ex-n
		case CLOCKINT_BIT: // Interrupcion de reloj = 9
